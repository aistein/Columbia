...
def optimizedMultiplyCUDA (A):
    ...
    M, N = A.shape
    TILE_WIDTH = 2 if ( M*M < 32 ) else 4 if ( M*M < 256 ) else 16 
    TILE_WIDTH = 2 if (TILE_WIDTH > N) else TILE_WIDTH;
    print "TILE_WIDTH: ", TILE_WIDTH
    build_options = []
    build_options.append('-DTILE_WIDTH')
    mod = SourceModule("""
        __global__ void multiply(const unsigned int *A, unsigned int *T, unsigned int *X, unsigned int M, unsigned int N)
        {
            // Define TILE_WIDTH (to be overridden in compile)
                #define TILE_WIDTH 2
            ...
            // Tiling Optimiziation: Pull tiles of size TILE_WIDTH x TILE_WIDTH into local memory before operation
                ...
                // Remember Row & Col are in reference to OUTPUT MATRIX!
                ...
                // Check against output grid dimensions
                if ( Row < M && Col < M ){
                    X[Row*M + Col] = xvalue;
                }
    ...
    b_size = (TILE_WIDTH,TILE_WIDTH,1)
    print b_size
    g = int(math.ceil(float(M*M)/float(TILE_WIDTH*TILE_WIDTH)/2.0))
    g_size = (g,g,1)
    print g_size

    ## Time the deployment of the kernel for metrics
    start = time.time()
    multiply(A_d, T_d, X_d, np.uint32(M), np.uint32(N), np.uint32(TILE_WIDTH), block=b_size, grid=g_size)
    runtime = time.time() - start
...
def main(_M_=5, _N_=5):
    ...
    if (not TEST_ALL):
        ...
        runtime = time.time() - start
        print "A * A_t: \n", A_m, "\n"
        print "is it symmetric? ", isSymmetric(A_m[1])
    ...
    if TEST_ALL :
        ...
        if MAKE_PLOT:
            ...
            plt.savefig('python_v_cuda_fix_times.png')
