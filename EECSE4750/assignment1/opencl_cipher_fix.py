...
def atbashEncodeOCL(input_string):
    ...
    program = cl.Program(context, """
    	__kernel void apply_cipher(__global const char *inputString, __global int *len, __global char *outputString)
    	{
    		// This input will be a 2-D matrix, so we only need three pieces of info to work with it (function of block grid)
    			...
    			int gid_y = get_global_id(0); // global ROW ID
    			int gdim_x = get_global_size(1); // global ROW dimensions
    			int gid_x = get_global_id(1); // global COLUMN ID
    ...
    global_dim = (int(math.ceil(length/float(max_size))), max_size)
    # local_dim = (max_size,1)
    local_dim = (1,max_size)
...
def main(_repetitions_=1):
    ...
    if (not TEST_ALL):
        ...
        ### Part 2: Implement the same using PyOpenCL.
        print "PART 2 (OPENCL): ", name," --> ", str(atbashEncodeOCL(name)[1])
        ...
        print "PART 3b (OPENCL): ", input_string," --> ", oclout
    ...
    if TEST_ALL :
    	...
    	if MAKE_PLOT:
    		...
    		plt.gca().set_ylim(0.0, (max(python_times) + max (ocl_times))*1.5)
    		plt.savefig('python_v_ocl_fix_times.png')
